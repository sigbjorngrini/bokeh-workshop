...
Use the ``bokeh serve`` command to run the example by executing:
    ...
    bokeh serve IV_fit.py

at your command prompt. Then navigate to the URL

    http://localhost:5006/IV_fit
...
def save_data():
    ...
    data = {'Voltage' : IV_data['Voltage'], 'Current' : IV_data['Current_fit_update']}
    IV_final_fit = pd.DataFrame(data = data,columns = ['Voltage','Current'],index = range(len(IV_data['Current_fit_update'])))
    
    #Save errors of fitting procedure
    if method == 'log' or method == 'linear':
        if method == 'log':
            var_residuals = np.var((np.log(np.abs(IV_data['Current']))-np.log(np.abs(V_data['Current_fit_update'])))**2)
        elif method == 'linear':
            var_residuals = np.var(np.abs(IV_data['Current']-np.abs(V_data['Current_fit_update'])**2))
        covar = var_residuals*hess_inv
        error = np.sqrt(covar.matvec(np.ones(len(params))))
        errors = {'Delta_I_0' : error[0], 'Delta_n' : error[1], 'Delta_R_S' : error[2], 'Delta_R_Sh' : error[3]}

        error_final = pd.DataFrame(data = errors, columns = ['Delta_I_0','Delta_n','Delta_R_S','Delta_R_Sh'],index = range(1))
    
    #Save output files
    IV_final_fit.to_csv('IV_fit_for_{0}K.txt'.format(str(T)), header=['Voltage','Current'], index=None, sep=' ', mode='w')
...
inputs_buttons = Column(children=[Fit_button,Save_button,methods_select,fit_methods_select])
